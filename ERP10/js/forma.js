...
function creador(datos){
	...
	for(let i=0;i<datos.shop.length;i++){
		...
		var coords = new Coords(datos.shop[i].coords.lat, datos.shop[i].coords.lng);
		...
		markers.push({"lat":coords.latitude, "lng":coords.longitude});
		console.log(sho.toString());
...
function showProduct(products){
	...
	while (product.done !== true){
		...
		img.setAttribute("id",product.value.product.SN)
...
function sesionIniciada(){
		...
		var eliminarDD = document.getElementById("eliminarDD");
	...
	function cerrarS(){
		...
		eliminarDD.setAttribute("class","hidden");
...
function drop(ev) {
    ...
    var data = ev.dataTransfer.getData("id");
  	console.log(data);
	  try{
	  	var proBorrar=sh.getProductAt(data);
		sh.removeProduct(proBorrar.product);

		dbdp(data);
		//alert("producto elimnado")
		mostrarProductos(tiendaActual.shop)
	  }catch(err){
		alert("solo se pueden borrar productos")
	  }
...
function mostrarMapa() {
	...
	var centerCoordinates = new google.maps.LatLng(LA, LON);
